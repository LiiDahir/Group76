...
class Main:
    ...
    def rename_files(self,folder_path="media/dataset/check/"):
        ...
        if  os.path.exists(self.folder_path):
            try:
                ...
                pass
        self.image_files = []
        count=self.count_files_in_folder( self.ddir)
        for file_name in os.listdir(self.folder_path):
            if os.path.isfile(os.path.join(self.folder_path, file_name)):
                ...
                if extension.lower() in self.image_extensions:
                    ...
                    count+=1
                    os.rename(self.folder_path+file_name,self.ddir+str(count)+".jpeg")
    ...
    def generate(self):
        ...
        for i in range(len(self.image_files)):
            ...
            sawir=self.ddir + self.image_files[i]
            x = self.recognize_face(sawir,0.5)
            print(x)
            if x == "No face found in the test image":
                count_remove+=1
            elif x==None:
                try:
                ...
                if db.count() == None:
                    ...
                    kii=int(db.count())
                IDga="C11800" + str(kii+1)
                ID.append(IDga)
                image = face_recognition.load_image_file(sawir)
                face_encoding = face_recognition.face_encodings(image)
                name_encodings.append(face_encoding[0])
                name_encodings = {"ID": ID, "encodings": name_encodings}
                with DEFAULT_ENCODINGS_PATH.open(mode="wb") as f:
                    pickle.dump(name_encodings, f)
                db.insert_data(IDga,full_name,fasal[last],6777,sawir)
                print("name_encodings : ",name_encodings,ID,IDga) 
    ...
    def recognize_face(self,test_image_path, distance_threshold=0.5):
        ...
        print("self.known_face_encodings waa : ",self.known_face_encodings)
